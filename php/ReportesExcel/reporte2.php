...
//	$paciente= $_GET['id'];
	...
	$strConsulta = "SELECT cod_profesor,nombre,apellidos from  profesor";
...
*/
  ...
  $pdf->SetTextColor(255);
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array('RFC', 'Nombre', 'Apellidos'));
			}
	...
	for ($i=0; $i<$numfilas; $i++)
		{
         ...
         $pdf->Cell(1,0,'',0,1);
			...
			if($i%2 == 1)
			{
    			...
    			$pdf->SetTextColor(0);
				$pdf->Row(array($fila['cod_profesor'], $fila['nombre'],$fila['apellidos']));
			}
			else
			{
				$pdf->SetFillColor(255,255,255);
