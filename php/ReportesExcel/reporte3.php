...
function Header()
{
  ...
  $this->Text(50,30,'Lista de Catedraticos Registrados en el sistema',0,'C', 0);
  $this->Ln(40);
}
...
//	$paciente= $_GET['id'];
  ...
  $num_control = $_SESSION['num_control'];
    $strConsulta ="SELECT id_materias,p_uno,p_dos,p_tres,p_cuatro, promedio from boleta where id_ncontrol='".$num_control."'";
...
*/
  ...
  $pdf->SetTextColor(255);
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array('Materia', 'parcial1', 'Parcial2','Parcial3','parcial4','promedio'));
			}
	...
	for ($i=0; $i<$numfilas; $i++)
		{
         ...
         $pdf->Cell(1,0,'',0,1);
			...
			if($i%2 == 1)
			{
    			...
    			$pdf->SetTextColor(0);
				$pdf->Row(array($fila['id_materias'], $fila['p_uno'],$fila['p_dos'],
                 $fila['p_tres'],$fila['p_cuatro'],$fila['promedio']));
			}
			else
			{
				$pdf->SetFillColor(255,255,255);
