...
function Header()
{
	...
	$this->Text(50,30,'Lista de Alumnos aprobados',0,'C', 0);
	$this->Ln(40);
}
...
//	$paciente= $_GET['id'];
  ...
  $strConsulta ="SELECT promedio,alumno.Nombre,alumno.Apellidos
...
*/
    ...
    $pdf->SetTextColor(255);
		...
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array('Promedio', 'Nombre', 'Apellidos'));
	...
	for ($i=0; $i<$numfilas; $i++)
		{
			...
			if($i%2 == 1)
			{
    			...
    			$pdf->SetTextColor(0);
				$pdf->Row(array($fila["promedio"], $fila["Nombre"],$fila["Apellidos"]));
			}
			else
			{
				$pdf->SetFillColor(255,255,255);
